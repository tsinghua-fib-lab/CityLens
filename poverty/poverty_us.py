...
from config import STV_NUM, ECONOMIC_US_NUM
random.seed(42)
...
def single_task_gen(city, task_name):
    ...
    task_path = f'/data5/liutianhui/UrbanSensing/data/poverty/{city}/{city}_poverty_{task_name}_task.json'
    ...
    task_map = {
        "median": "income",
        "line_100": "povertyline_below100",
        "line_200": "povertyline_below200",
    }
    all_data = []
...
def generate_session_simple(city, data, task_name):
    """
    根据输入的数据生成交替的文本和图片（url）结构，并返回prompt数据
    """
    url_file = f"/data5/liutianhui/ossutil-2.0.3/url_mapping_US_{city}_20250507_1month.csv"
    df_url = pd.read_csv(url_file)
    url_dict = dict(zip(df_url['image_name'], df_url['image_url']))
    task_indicator_map = {
        "median": "median household income",
        "line_100": "percentage of individuals below the 100% poverty line",
        "line_200": "percentage of individuals below the 200% poverty line",
    }
    example_map = {
        "median": "124129",
        "line_100": "12.9",
    ...
    prompt = f"Suppose you are a professional socio-economic data analyst in {city}, United States. Based on the provided satellite imagery and several street view photos, please estimate 'the {indicator}' in the census tract where these images are taken. Consider factors such as local infrastructure, economic activity, neighborhood conditions, and any other relevant details."
...
def generate_session_map(city, data, task_name):
    ...
    prompt = f"Suppose you are a professional socio-economic data analyst in {city}, United States. Based on the provided satellite imagery and several street view photos(with their corresponding addresses and nearby places), please estimate 'the {indicator}' in the census tract where these images are taken. Consider factors such as local infrastructure, economic activity, neighborhood conditions, and any other relevant details."
...
def single_eval_task(args):
    ...
    if prompt_type == "simple":
        session = generate_session_simple(city, d, task_name)
...
def eval_task(city, model_name, num_process, prompt_type, task_name):
    ...
    response_path = f'/data5/liutianhui/UrbanSensing/results/poverty/{task_name}/{city}/{model_name_full}_{prompt_type}_response.json'
    ...
    with open(task_path, "r") as f:
        data = json.load(f)
    if len(data) > ECONOMIC_US_NUM:
        data = random.sample(data, ECONOMIC_US_NUM)
...
def main():
    ...
    parser.add_argument('--num_process', type=int, default=5, help='number of processes for evaluation')
