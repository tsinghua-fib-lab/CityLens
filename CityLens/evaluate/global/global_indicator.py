...
def single_task_gen(city, task_name):
    data_path = f'/data5/liutianhui/UrbanSensing/data/remote_sensing/{city}_img_indicators.csv'
    sat_stv_path = f"/data5/liutianhui/UrbanSensing/data/street_view/{city}/all_stitch_meta_info.csv"
    stv_folder = f"/data5/liutianhui/UrbanSensing/data/street_view/{city}/images/"
    sat_folder = f"/data5/liutianhui/UrbanSensing/data/remote_sensing/{city}/"
    task_path = f'/data5/liutianhui/UrbanSensing/data/global/{city}/{city}_{task_name}_task.json'
...
def generate_session_map(city, data, task_name):
    ...
    prompt = f"Suppose you are a professional real estate appraisal expert in {city}, United States. Based on the provided satellite imagery and several street view photos(with their corresponding addresses and nearby places), please estimate 'the {indicator}' in the census tract where the images are taken. Consider factors such as location, visible property features, neighborhood condition, and any other relevant details."
    image_info_dict = {}
...
def eval_task(city, model_name, num_process, prompt_type, task_name):
    if city == "all":
        task_path = f"/data5/liutianhui/UrbanSensing/data/global/all_global_{task_name}_task.json"
    else:
        task_path = f'/data5/liutianhui/UrbanSensing/data/global/{city}/{city}_{task_name}_task.json'
    
    model_name_full = model_name.replace("/", "_")
    response_path = f'/data5/liutianhui/UrbanSensing/results/global/{task_name}/{city}/{model_name_full}_{prompt_type}_response.json'
