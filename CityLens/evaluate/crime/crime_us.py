...
def data_gen_simple(city, task_name):
    ...
    task_path = f'/data5/liutianhui/UrbanSensing/data/crime/{city}/{city}_crime_{task_name}_task.json'
...
def generate_session_simple(city, data, task_name, prompt_type, model_name):
    ...
    for i, image_path in enumerate(images):
        ...
        if i == 0:
            ...
            # 对于后续图片，添加 street view image {i} 的描述，并插入其url
            content.append({
                "type": "text",
...
def generate_session_map(city, data, task_name):
    ...
    prompt = f"Suppose you are a professional crime data analyst in {city}, United States. Based on the provided satellite imagery and several street view photos(with their corresponding addresses and nearby places), please estimate the occurrence of {task_name} crime in the census tract where these images are taken. Consider factors such as location, visible property features, neighborhood condition, and any other relevant details that might influence the occurrence of {task_name} crime in the area."
    image_info_dict = {}
    ...
    content.append({
        "type": "text",
        "text": "Please provide a single, exact number only (not a range). No explanation is needed.\n Example answer: 12"
    })
...
def eval_task(city, model_name, num_process, prompt_type, task_name):
    if city == "US":
        task_path = f"/data5/liutianhui/UrbanSensing/data/crime/US_crime_{task_name}_task.json"
    else:
        task_path = f'/data5/liutianhui/UrbanSensing/data/crime/{city}_crime_{task_name}_task.json'
    
    
    model_name_full = model_name.replace("/", "_")
    response_path = f'/data5/liutianhui/UrbanSensing/results/crime/{task_name}/{city}/{model_name_full}_{prompt_type}_response.json'
