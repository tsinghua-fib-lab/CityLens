...
def data_gen_simple(city):
    data_path = f'/data5/liutianhui/UrbanSensing/data/life_expectancy/{city}_life_expectancy_msoa_male.csv'
    msoa_sat_stv_path = f'/data5/liutianhui/UrbanSensing/data/UK_image/sat/{city}_msoa_sat_stv.json'
    task_path = f'/data5/liutianhui/UrbanSensing/data/life_expectancy/{city}/{city}_life_expectancy_task.json'
    ...
    prefix = f'/data5/liutianhui/UrbanSensing/data/UK_image/sat/{city}/'
    ...
    for _, row in df.iterrows():
        msoa = row['MSOACode']  # 确保 key 是字符串
        if msoa not in msoa_sat_stv:
            # print(f"msoa {msoa} 不在 JSON 里，跳过")
            continue
        ...
        if len(stv_paths_all) > STV_NUM:
            ...
            print(f"msoa {msoa} 的街景图片数量不足，使用全部 {len(stv_paths_all)} 张")
...
def eval_task(city, model_name, num_process, prompt_type):
    if city == "UK":
        task_path = f"/data5/liutianhui/UrbanSensing/data/life_expectancy/UK_life_expectancy_task.json"
    else:
        task_path = f'/data5/liutianhui/UrbanSensing/data/life_expectancy/{city}_life_expectancy_task.json'
    model_name_full = model_name.replace("/", "_")
    response_path = f'/data5/liutianhui/UrbanSensing/results/life_expectancy/{city}/{model_name_full}_{prompt_type}_life_expectancy_response.json'
